...
public:
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    if(m_parent->m_pszDbgHelpLib == NULL)
    {
    ...
    {
      m_hDbhHelp = LoadLibrary(m_parent->m_pszDbgHelpLib);
    }
    if(m_hDbhHelp == NULL) 
    {
...
StackWalker::StackWalker(DWORD dwProcessId, HANDLE hProcess, LPCTSTR szSymPath, LPCTSTR pszDbgHelpLib)
{
  this->m_options = OptionsAll;
  this->m_modulesLoaded = FALSE;
  this->m_hProcess = hProcess;
  ...
  lstrcpy(this->m_pszDbgHelpLib,pszDbgHelpLib);
  if (szSymPath != NULL)
  {
    lstrcpy(this->m_szSymPath,szSymPath);
...
StackWalker::StackWalker(int options, LPCSTR szSymPath, LPCTSTR pszDbgHelpLib, DWORD dwProcessId, HANDLE hProcess)
{
...
BOOL StackWalker::LoadModules(PCSTR pszSymPath) // split the path by ";". 
{
  if (this->m_sw == NULL)
  {
    SetLastError(ERROR_DLL_INIT_FAILED);
  ...
  // Build the sym-path:
  TCHAR *szSymPath = (TCHAR*)pszSymPath;
  if ( (this->m_options & SymBuildPath) != 0)
  {
    if(pszSymPath == NULL)
    {
      ...
      if (GetModuleFileNameA(NULL, szTemp, nTempLen) > 0)
      {
        ...
        if (strlen(szTemp) > 0)
        {
          ...
          strcat_s(szSymPath, nSymPathLen, ";");
          strcat_s(szSymPath, nSymPathLen, szTemp);
          strcat_s(szSymPath, nSymPathLen, "\\debug;");
        }
      }
      ...
      if (GetEnvironmentVariableA("SYSTEMROOT", szTemp, nTempLen) > 0) // C:\Windows 
      {
        szTemp[nTempLen-1] = 0;
        strcat_s(szSymPath, nSymPathLen, szTemp);
        strcat_s(szSymPath, nSymPathLen, ";");
    ...
    if(pszSymPath == NULL)
    {
      if ( (this->m_options & SymBuildPath) != 0)
      {
        ...
        if (GetEnvironmentVariableA("SYSTEMDRIVE", szTemp, nTempLen) > 0) // C: 
        {
          szTemp[nTempLen-1] = 0;
...
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
  _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Symbol-SearchPath: '%s', symOptions: %d, UserName: '%s'\n", szSearchPath, symOptions, szUserName);
  OnOutputInitSym(buffer);
...
void StackWalker::OnOutputCallStack(LPCSTR buffer)
{
  #ifdef _DEBUG
  OutputDebugStringA(buffer);
  #endif 
...
void StackWalker::OnOutputInitSym(LPCSTR buffer)
{
  #ifdef _DEBUG
  OutputDebugStringA(buffer);
  #endif 
...
public:
  ...
  MyStackWalker(DWORD dwProcessId, HANDLE hProcess, LPCSTR szSymPath, PCSTR pszDbgHelpLib) 
    : StackWalker(dwProcessId, hProcess, szSymPath, pszDbgHelpLib)
  {
    
  }
  MyStackWalker(LPCSTR szSymPath, PCSTR pszDbgHelpLib) 
    :  StackWalker(OptionsAll, szSymPath, pszDbgHelpLib, GetCurrentProcessId(), GetCurrentProcess())
  {
    
  }
  
  virtual void OnOutput(LPCSTR szText) { /*fprintf(stdout,szText); */  StackWalker::OnOutput(szText); }
  virtual void OnOutputCallStack(LPCSTR szText) {  fprintf(stdout,szText); }
};

#if 0 
typedef void (*PFNOUTPUTCALLSTACK)(LPCSTR szText);

U32 StackWalk(DWORD dwProcessId, HANDLE hProcess, 
              LPCSTR szSymPath, PCSTR pszDbgHelpLib,
              PFNOUTPUTCALLSTACK pfnOutputCallStack) 
{
  StackWalker sw(OptionsAll, szSymPath, pszDbgHelpLib, szSymPath, pszDbgHelpLib);
}
#endif 
