...
#if HAVE_QUEUE_DEBUG 
#ifdef _DEBUG
#define _ASSERTE(x) if(!(x)) __asm { int 3 }
#else
#define _ASSERTE(x) {}
#endif
#else
...
LONG CXQueue::Close()
{
	...
	if (IsOpen())
	{
		if (m_bClient)
		{
			if (m_pVariableBlock && m_pVariableBlock->NoClients > 0)
				m_pVariableBlock->NoClients--;
		}
		else
		{
			if (m_pVariableBlock && m_pVariableBlock->NoServers > 0)
...
// Check the queue is open or not.
BOOL CXQueue::CheckOpen(LPCTSTR lpszQueueName)
{
	if (IsOpen())
	{
		return TRUE;
...
//              pdwThreadId  - [out] the thread id of the thread that queued
//                             the message
//              lpData       - [out] Pointer to receive the address.
//              lpcbSize     - [in/out] size in bytes of lpData;  if lpData
//                             is NULL, the size of the required buffer will
//                             returned in lpcbSize.
//              dwTimeOut    - [in] specifies the time-out interval, in
...
// Returns:     LONG         - XQueue error code (see XQueue.h)
//
DWORD CXQueue::Read(size_t * lpnMessageId,
                   DWORD  * pdwThreadId,
                   LPBYTE* lpData,
                   size_t * lpcbSize,
                   DWORD    dwTimeOut /*= INFINITE*/)
{
  ...
  *lpcbSize     = 0;
  ...
  if (m_pVariableBlock->FreeDataBlocks < m_pVariableBlock->NoDataBlocks)
  {
    size_t block_no = m_pVariableBlock->UsedListForwardLink;
    if (block_no != XQUEUE_INVALID_BLOCK)
    {
      ...
      *lpcbSize = GetEntrySize(pXDB);
      ...
      DWORD dwSize = *lpcbSize;
      LPBYTE pBuffer = (LPBYTE)malloc(dwSize);
      if(pBuffer)
      {
        // caller wants the data
        LONG lRet = ReadMMF(pBuffer, lpcbSize, dwTimeOut);
        if(lRet == XQUEUE_ERROR_SUCCESS)
        {
          *lpData = pBuffer;
        }
        else if(lRet == XQUEUE_ERROR_TIMEOUT) // wait writing Mutex.
        {
          *lpData = pBuffer;
          
          // Try again.
          lRet = UpdateReadMMF(dwTimeOut);
          if(lRet != XQUEUE_ERROR_SUCCESS)
          {
             dwRet = ERR_TIMEOUT_I;
             free(pBuffer);
        ...
        else
        {
          ...
          *lpcbSize = 0;
          ...
          else if(lRet == XQUEUE_ERROR_PARAMETER)
          {
...
DWORD CXQueue::ReadEx(size_t * lpnMessageId,
                   DWORD  * pdwThreadId,
                   LPBYTE lpData, // out 
                   DWORD * lpcbSize, // in/out
                   DWORD    dwTimeOut /*= INFINITE*/)
{
  DWORD dwRet = 0;
  DWORD dwDataSize =0;
  ...
  *lpcbSize     = 0;
  ...
  if (m_pVariableBlock->FreeDataBlocks < m_pVariableBlock->NoDataBlocks)
  {
    ...
    if (block_no != XQUEUE_INVALID_BLOCK)
    {
      ...
      // caller just wants the size
      *lpcbSize = GetEntrySize(pXDB);
      if(*lpcbSize > dwDataSize)
      {
        return ERR_BUFFER_TOO_SMALL_I;
      }
      TRACE(_T("GetEntrySize returned %d for block %d\n"), *lpcbSize, block_no);
      
      DWORD dwSize = *lpcbSize;
      UNREF_PARAM(dwSize);
      LPBYTE pBuffer = lpData;
      if(pBuffer)
      {
        // caller wants the data
        LONG lRet = ReadMMF(pBuffer, (unsigned int*)lpcbSize, dwTimeOut);
        if(lRet == XQUEUE_ERROR_SUCCESS)
        {
        }
        else if(lRet == XQUEUE_ERROR_TIMEOUT) // wait writing Mutex.
					...
					DWORD dwTimeOut /*= INFINITE*/)
{
	...
	else
	{
        ...
        if(m_pVariableBlock->MessageId != 0xFFFFFFFF)
          m_pVariableBlock->MessageId++;

        // wrapp 
        if(m_pVariableBlock->MessageId >= 0xFFFFFFFF-1)
          m_pVariableBlock->MessageId = 1;
			}
		}
    ...
    {
      OutputDebugString(_T("*** Fail to Write MMF.\n"));
    }
	}
...
class QueueCriticalSection
{
  CRITICAL_SECTION m_CriticalSection;
  
public:
  QueueCriticalSection()     
  {
...
PRIVATE QueueCriticalSection g_QCs;

U32 CXQueue::ShutdownSignal(DWORD dwTimeOut)
{
  UNREF_PARAM(dwTimeOut);
  ...
  {
    g_QCs.Leave();
    return ERR_NO_SERVER_I;
  }
  
  m_pVariableBlock->MessageId      = 0xffffffff;
  m_pVariableBlock->FreeDataBlocks = 0;
  m_pVariableBlock->NoServers      = 0;
  if (m_hServerNotifyEvent)
  {
    SetEvent(m_hServerNotifyEvent);
  }
...
LONG CXQueue::UpdateReadMMF(DWORD dwTimeOut)
{
  LONG lRet = 0;
...
LONG CXQueue::WriteToMMF(LPBYTE lpData, size_t nDataSize)
{
  ...
  LPBYTE pData   =  lpData;
	...
	while (lDataSize > 0)
	{
		...
		size_t nBytesToCopy = ((size_t)lDataSize > m_nDataSize) ?
								m_nDataSize : lDataSize;

		memcpy(pXDB->Data, pData, nBytesToCopy);
...
LONG CXQueue::CreateSyncObjects()
{
...
#if 0 
  HANDLE hWriteMutex = OpenMutex(MUTEX_ALL_ACCESS, 0, szName);
  if(hWriteMutex)
    ::CloseHandle(hWriteMutex);
#endif // shenhankun/2009-05-01 
  ...
  HANDLE hSrvEvent = OpenEvent(EVENT_ALL_ACCESS, 0, szName);
  if(hSrvEvent)
    ::CloseHandle(hSrvEvent);
  ...
  HANDLE ClientWriteEvent = OpenEvent(EVENT_ALL_ACCESS, 0, szName);
  if(ClientWriteEvent)
    ::CloseHandle(ClientWriteEvent);
...
BOOL CXQueue::IsEnoughFreeBlocks(size_t nDataSize)
{
 return (m_pVariableBlock->FreeDataBlocks >= CalculateNoBlocks(nDataSize));
}

BOOL  CXQueue::IsValidDataSize(size_t nDataSize)
{
  return (nDataSize < (m_nBlockSize * (m_nQueueSize-1)));
}

DWORD CXQueue::QueueClear(DWORD dwOption/*=0*/)
{
  UNREF_PARAM(dwOption);
