...
#define XQUEUE_DATA_VARIABLE_SIZE	(7 * sizeof(size_t))	// no. of size_t variables in a data block
...
public:
  ...
  DWORD Read(size_t * lpnMessageId,
    DWORD * pdwThreadId,
    LPBYTE* lpData,
    size_t * lpcbSize,
    DWORD dwTimeOut= INFINITE);
  ...
  BOOL  IsValidDataSize(size_t nDataSize) ;
  BOOL  IsEnoughFreeBlocks(size_t nDataSize) ;
  BOOL  CheckOpen(LPCTSTR lpszQueueName);
  DWORD ShutdownSignal(DWORD dwTimeOut=INFINITE);
  DWORD QueueClear(DWORD dwOption=0);

  // Implementation
private:
  void Add(size_t & ForwardLink, size_t & BackLink, size_t & BlockCount, XQUEUE_DATA_BLOCK* pXDB);
  ...
  XQUEUE_DATA_BLOCK* GetBlock(size_t & ForwardLink, size_t & BackLink, size_t & BlockCount);
  ...
  LONG	ReadMMF(LPBYTE lpData, size_t * lpcbSize, DWORD dwTimeOut);
  LONG	ReleaseEntry();
  LONG	SetQueueAvailable();
  LONG	Unlock();
  LONG	WaitForQueueIdle(DWORD dwTimeOut);
  LONG	WriteToMMF(LPBYTE lpData, size_t nDataSize);
  LONG  UpdateReadMMF(DWORD dwTimeOut);
  VOID InitQueue();
