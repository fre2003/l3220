...
U32 CPing_i::Ping(LPCTSTR pstrHost, UINT nRetries, U32 ulTimeOut)
{
  U32 ret = 0;
  CString strErr;
  CString strPingResult;

  ret = Ping(pstrHost, strErr, strPingResult, nRetries, ulTimeOut);
  
  return ret;
}


U32 CPing_i::Ping(LPCTSTR pstrHost, OUT CString& strErr, OUT CString& strPingResult, UINT nRetries, U32 ulTimeOut)
{
  U32 ret = 0;
  ...
  if (p1.Ping1(pstrHost, pr1, 128, ulTimeOut, 32))
  {
    strErr = _T("");
    strPingResult.Format(_T("Reply from: %d.%d.%d.%d bytes=32 time=%ldms TTL=128"), 
  ...
  {
    strErr.Format(_T("Failed in call to ping, GetLastError returns: %d"), GetLastError());
    ret = ERR_TIMEOUT_I;
  }  
