...
static CString GetAppPath(VOID) 
{
  ...
  filepath[0] = _T('\0');
  GetModuleFileName(AfxGetApp()->m_hInstance,filepath, MAX_PATH); 
...
CString	CPath::GetShortPath()
{
	...
	//GetShortPathName(GetPath(), szShortPath, MAX_PATH);
...
CString	CPath::GetLongPath()
{
	...
	//GetLongPathName(GetPath(), szLongPath, MAX_PATH);
...
{
	if (_sDir.IsEmpty() || (_aDir.GetSize() > 0)) 
    return;
	...
	while ((nTo = _sDir.Find(_T('\\'), nFrom)) != -1)
	{
...
void CPath::FillArgArray()
{
	...
	{
		bQuotedArg = (sArgs[nTo] == _T('\"'));

		if (bQuotedArg)
		{
			nFrom = nTo + 1;
			if ((nTo = sArgs.Find(_T('\"'), nFrom)) == -1) break;
