// StdioFileEx.cpp: implementation of the CStdioFile_i class.
//
...
/*static*/ const UINT CStdioFile_i::modeWriteUnicode = 0x20000; // Add this flag to write in Unicode

  
static int		GetUnicodeStringFromMultiByteString(
  IN LPCSTR szMultiByteString, 
  OUT wchar_t* szUnicodeString, 
  IN int nUnicodeBufferSize, IN int nCodePage=-1);
...
CStdioFile_i::CStdioFile_i(): CStdioFile()
{
...
void CStdioFile_i::SetCodePage(IN const UINT nCodePage)
{
...
BOOL CStdioFile_i::ReadString(CString& rString)
{
...
//					CStdioFile_i::modeWriteUnicode flag. Override of base class function.
...
// Exceptions:	None.
//
void CStdioFile_i::WriteString(LPCTSTR lpsz)
{
  wchar_t*	pszUnicodeString	= NULL; 
  char	*	pszMultiByteString= NULL;
  ...
  try
  {
    ...
    if (m_nFlags & CStdioFile_i::modeWriteUnicode)
    {
...
UINT CStdioFile_i::ProcessFlags(const CString& sFilePath, UINT& nOpenFlags)
{
  m_bIsUnicodeText = false;
  
  // If we have writeUnicode we must have write or writeRead as well
#ifdef _DEBUG
  if (nOpenFlags & CStdioFile_i::modeWriteUnicode)
  {
...
// Exceptions:	None.
//
/*static*/ bool CStdioFile_i::IsFileUnicode(const CString& sFilePath)
{
  CFile				file;
...
unsigned long CStdioFile_i::GetCharCount()
{
  int				nCharSize;
...
// --------------------------------------------------------------------------------------------
//
//	CStdioFile_i::GetUnicodeStringFromMultiByteString()
//
// --------------------------------------------------------------------------------------------
...
// Exceptions:	None.
//
int GetUnicodeStringFromMultiByteString(IN LPCSTR szMultiByteString, OUT wchar_t* szUnicodeString, IN int nUnicodeBufferSize, IN int nCodePage)
{
  bool		bOK = true;
  int		nCharsWritten = 0;
...
// --------------------------------------------------------------------------------------------
//
//	CStdioFile_i::GetMultiByteStringFromUnicodeString()
//
// --------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------
//
//	CStdioFile_i::GetRequiredMultiByteLengthForUnicodeString()
//
//---------------------------------------------------------------------------------------------------
// Returns:    int
