...
void  CPdhCounter::SetFullCounterPath(LPCTSTR pszFullCounterPath)
{
  m_sCounterPath = pszFullCounterPath;
}
...
void CPdhCounter::GetInfo(CPdhCounterInfo& info)
{
  ...
  {
    //Call again with the newly allocated buffer
    PPDH_COUNTER_INFO pszBuffer = (PPDH_COUNTER_INFO) new BYTE[dwBufferSize+10];
...
BOOL CPdh::ValidatePath(LPCTSTR pszFullCounterPath)
{
...
void CPdh::ExpandPath(LPCTSTR pszWildCardPath, CStringArray& expandedPaths)
{
  ...
  PDH_STATUS Status = PdhExpandCounterPath(pszWildCardPath, NULL, &dwBufferSize);
...
void CPdh::ParseInstanceName(LPCTSTR pszInstanceString, CString& sInstanceName, CString& sParentName, DWORD& Index)
{
  ...
  {
    //Call again with the newly allocated buffer
    LPTSTR pszInstanceName = (LPTSTR) new TCHAR[dwInstanceNameBufferSize+1];
    LPTSTR pszParentName = (LPTSTR) new TCHAR[dwParentNameBufferSize+1];
