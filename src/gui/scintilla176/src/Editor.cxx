...
Editor::Editor() 
{
  ...
  allowkeyinput = 1;
  CtrlChar = 0;
  doclimitsize = 10*1024*1024; // 10M initial size
...
const char* Editor::ControlCharacterString(unsigned char ch) 
{
	const char *reps[] = 
  {
  ...
  if(CtrlChar == 2)
  {
    if (ch < (sizeof(szCtrChar) / sizeof(szCtrChar[0]))) 
    {
      return szCtrChar[ch];
    }  
  }
  ...
  {
    if (ch < (sizeof(szCmdCtrChar) / sizeof(szCmdCtrChar[0]))) 
    {
      return szCmdCtrChar[ch];
    } 
  }
  else 
  {
...
void Editor::NewLine() 
{
  ...
  if(cmdmode && m_KeyQueue.IsCmdHistory() )
  {
    char buf[MAX_INPUT_SIZE+1] = {0, };
    ...
    if(m_KeyQueue.IsCmdHistory())
    {
      if ((unsigned int)lineStart == lineEnd && strlen(m_KeyQueue.m_lastCmd) > 0)
      {
        strcpy(buf, m_KeyQueue.m_lastCmd);
        pdoc->InsertString(CurrentPosition(), buf, strlen(buf));
        MovePositionTo(pdoc->Length());
        SetLastXChosen();
        lineCurrentPos = pdoc->LineFromPosition(currentPos);
        lineStart      = pdoc->LineStart(lineCurrentPos);
        lineEnd        = pdoc->LineStart(lineCurrentPos + 1);   
      }
    }
    
    char* ptr           = buf; // CharPtrFromSPtr(lParam);
    unsigned int iPlace = 0;
    unsigned int wParam = MAX_INPUT_SIZE;
    for (unsigned int iChar = lineStart; iChar < lineEnd && iPlace < wParam - 1; iChar++)
    {
      ptr[iPlace++] = pdoc->CharAt(iChar);
    }
    
    iPreNewLinePos = currentPos;
    
    ptr[iPlace] = '\0';
    unsigned int len   = currentPos - lineStart;
    SCNotification scn = { 0 };
    
    scn.message     = SCN_NEWLINE;
    scn.nmhdr.code  = SCN_NEWLINE;
    scn.pNewLine    = ptr;
    NotifyParent(scn);    
#else
    //if(currentPos < iPreNewLinePos) 
      //iPreNewLinePos = currentPos;
    int line1 = pdoc->LineFromPosition(iPreNewLinePos);
    int start = pdoc->LineStart(line1);
    int line2 = pdoc->LineFromPosition(currentPos);
    int end = pdoc->LineEnd(line2);
    char *psztext = CopyRange(start, end);
    SCNotification scn = { 0 };
    scn.message     = SCN_NEWLINE;
    scn.nmhdr.code  = SCN_NEWLINE;
    scn.pNewLine    = psztext;
    NotifyParent(scn);
    
    delete psztext;
    iPreNewLinePos = currentPos;
#endif
    m_KeyQueue.TrimString(buf); // remove \t \r \n space.
    if(strlen(buf) > 0)
    {
      INPUT_KEY key;
      strcpy(key.cmdLine, buf);
      strcpy(m_KeyQueue.m_lastCmd, key.cmdLine);
      key.status = 1; 
      m_KeyQueue.PostKeyQueue(&key); 
    }
  } // Command History.
  else 
  {
...
void Editor::ReplaceCurrentLine(const char* s, int insertLength)
{
  int line  = pdoc->LineFromPosition(currentPos);
  int start = pdoc->LineStart(line);
  int end   = pdoc->LineStart(line + 1);
  
  if (pdoc->DeleteChars(start, end - start))
  {    
    pdoc->InsertString(CurrentPosition(), s, insertLength);
    SetEmptySelection(currentPos + insertLength);
  }
  else 
  {
    pdoc->InsertString(CurrentPosition(), s, insertLength);
	...
	switch (iMessage) 
  {
	  ...
	  CursorUpOrDown(1);
    {
      if (cmdmode && m_KeyQueue.IsCmdHistory())
      {
        ...
        m_KeyQueue.PeekKeyQueue(&key, 0);
        if (strlen(key.cmdLine) > 0)
        {
          ReplaceCurrentLine(key.cmdLine, strlen(key.cmdLine));
        }
        MovePositionTo(pdoc->Length());
        SetLastXChosen();
      }
  ...
  case SCI_DELETEBACK:
    {
      ...
      if (cmdmode)
      {
        int line       = pdoc->LineFromPosition(currentPos);
        int start      = pdoc->LineStart(line);
        int totalLines = pdoc->LinesTotal();
        line += 1;
        ...
        if (currentPos == start) // in same line.
        {
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
  ...
  case SCI_SETTEXT:
    ...
    if(cmdmode)
    {
      int line       = pdoc->LineFromPosition(currentPos);
      int start      = pdoc->LineStart(line);      
      if (currentPos != start) // allow to paste at last line(currnetl ine).
      {
        MovePositionTo(pdoc->Length());
        SetLastXChosen();
      }
    ...
    if (cmdmode)
    {
      ...
      if (currentPos != start)    // in same line.
      {
        if (currentPos == anchor)   // No Selection
        {
          MovePositionTo(pdoc->Length()); // move to last line end position.
          SetLastXChosen();
          return 0;
        }
      }
    ...
    //pdoc->InsertChar(pdoc->Length(), 0x1b);
		return 0;

	case SCI_LINESTART_TEXT:
    { 
      MovePositionTo(pdoc->Length());
      int line       = pdoc->LineFromPosition(currentPos);
      int start      = pdoc->LineStart(line);
      pdoc->InsertString(start, CharPtrFromSPtr(lParam), wParam);
      MovePositionTo(pdoc->Length());
    }
		return 0;
  ...
  case SCI_SET_CTRLCHAR_MODE:
    {
      CtrlChar = wParam>0?1:0;      
    }
   break;

  case SCI_INSERT_CTRLCHAR:
    {
       int pos = lParam;
       char ch = wParam;
       if(pos == 0) 
       ...
       pdoc->InsertChar(pos, ch); // insert char 
       SetEmptySelection(currentPos + 1); // move to last 
    }
   break;  

  case SCI_CMD_MODE:
    {
      cmdmode = wParam>0?1:0; 
      if(cmdmode)
      {
        convertPastes = false;
      }
      ...
      {
        posCache.SetSize(0x400); // the default value.
      }
    }
   break;

  case SCI_CMD_KEY_INTPUT_MODE:
    {
      allowkeyinput = wParam>0?1:0;  
    }
   break;
  
  case SCI_CMD_LIMIT_TEXT_SIZE:
    {
      if(cmdmode && allowkeyinput)
      {
        doclimitsize = wParam;
      }
      else 
      {
        doclimitsize = 10*1024*1024; // 10 MBytes
      }
    }
   break;
  
  case SCI_CMD_HISTORY:
    {
      if(cmdmode && allowkeyinput)
      {
        if(wParam == 1)
        {
          m_KeyQueue.LoadQueue((const char*)lParam);
        }
        else if(wParam == 2)
        {
           m_KeyQueue.SaveQueue();
        }
      }
      else 
      {
  ...
  case SCI_OUTPUT_MODE:
    outputmode = wParam>0?1:0;
    break;
