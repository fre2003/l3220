...
//UINT g_NotifyNewLineWndMessage = ::RegisterWindowMessageA(("__NOTIFY_WND_NEW_LINE_MESSAGE__"));
...
class ScintillaWin :
	...
	virtual unsigned int NotifyParent(SCNotification scn);
	virtual unsigned int GotoDocuemntEnd(VOID);
	virtual unsigned int IsCurrentLine(VOID); 
...
static int KeyTranslate(int keyIn, int modifiers) 
{
	switch (keyIn) {
		...
		case VK_RETURN:	
#if  0
      if(modifiers&SCI_ALT) 
         return SCI_LINESSPLIT;
      else 
#endif 
        return SCK_RETURN; // new line 
...
sptr_t ScintillaWin::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) 
{
	...
	switch (iMessage) 
  {
    ...
    // Register OLE drag-and-drop operation  
  ...
  case WM_KEYDOWN: 
    {
      ...
      int modifiers = (Platform::IsKeyDown(VK_SHIFT) ? SCI_SHIFT : 0) |
          (Platform::IsKeyDown(VK_CONTROL) ? SCI_CTRL : 0) | (Platform::IsKeyDown(VK_MENU) ? SCI_ALT : 0);
      
      // Command Mode
      if (cmdmode)
      {
#if  1
        if((modifiers & SCI_CTRL) && wParam == 'C')
        {
          wParam = 0x03;
        }
        if((modifiers & SCI_CTRL) && wParam == 'Z')
        {
          wParam = 0x1A; // 
        }
        //else 
#endif 
        {
          if (!((wParam == VK_CONTROL || wParam == VK_SHIFT || wParam == VK_MENU || wParam == VK_BACK
            || wParam == VK_DELETE) || ( wParam > VK_F1 && wParam <VK_F24) || 
            (wParam > VK_SPACE && wParam < VK_HELP)) )
          {
            if(!IsCurrentLine()) 
            {
               GotoDocuemntEnd();
      ...
      lastKeyDownConsumed = false;
      int ret = KeyDown(KeyTranslate(wParam, modifiers),
        Platform::IsKeyDown(VK_SHIFT),
        Platform::IsKeyDown(VK_CONTROL),
        Platform::IsKeyDown(VK_MENU),
        &lastKeyDownConsumed);
...
unsigned int ScintillaWin::NotifyParent(SCNotification scn) {
	scn.nmhdr.hwndFrom = MainHWND();
	scn.nmhdr.idFrom = GetCtrlID();
	return ::SendMessage(::GetParent(MainHWND()), WM_NOTIFY,
	              GetCtrlID(), reinterpret_cast<LPARAM>(&scn));
}

unsigned int  ScintillaWin::GotoDocuemntEnd() 
{
  if(cmdmode)
  {
...
unsigned int  ScintillaWin::IsCurrentLine() 
{
  int line       = pdoc->LineFromPosition(currentPos);
  int start      = pdoc->LineStart(line);
